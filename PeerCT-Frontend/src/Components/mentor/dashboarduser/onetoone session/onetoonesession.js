...
export const OneToOneSession=()=>
{
    ...
    useEffect(async () => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/mentor/serviceDetailByUser/new`,
                { headers: { Authorization: `Bearer ${token}` } })
            const data = await response.data
            setUpData(data)
            const response2 = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/mentor/serviceDetailByUser/old`,
                { headers: { Authorization: `Bearer ${token}` } })
