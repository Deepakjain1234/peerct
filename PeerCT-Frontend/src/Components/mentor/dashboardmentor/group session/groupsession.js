...
export const GroupSession=()=>
{
    ...
    useEffect(async () => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/GroupSession/upcoming`,
                { headers: { Authorization: `Bearer ${token}` } })
            const data = await response.data
            setUpData(data)
            const response2 = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/GroupSession/completed`,
                { headers: { Authorization: `Bearer ${token}` } })
            ...
            }
             {
                ...
                
                {
                    completed.map((element) => {
                        return (<SessionCompleted  id={element.id} topic={element.topic} date={element.date} time={element.star_time} attendee={element.attendee} medium={element.medium}  />)
                    })
                }
